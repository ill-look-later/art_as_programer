...
```
　　其实这段代码也让我之后做的一些小折腾变得可能。直接load一个另外一个功能模块就可以轻松地扩展这个应用。借助于qt的特性。这个source还可以是网络上的qml文件。所以真TMD方便。关键就这么几句话呀。用我这中习惯C家族的死脑筋真的是看的目瞪口呆呀...{^&^}（ps: 其实早就习惯了qml得这个特性...）main.qml 中那些**FeedManagerView** **ArticleListView**这些只不过是一些视图模块而已。